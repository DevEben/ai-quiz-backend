...
export async function POST(req: Request) {
  ...
  let questionsToGenerate: { subject: string; numQuestions: number }[] = [];
  if (mode === "practice") {
    if (!numberOfQuestions) {
      return NextResponse.json({ error: "numberOfQuestions required for practice mode" }, { status: 400 });
    }
    if (subjects.length !== 1) {
      return NextResponse.json({ error: "Practice mode requires exactly one subject" }, { status: 400 });
    }
    questionsToGenerate = [{ subject: subjects[0], numQuestions: numberOfQuestions }];
  } else if (mode === "ExamMode" && examType === "JAMB") {
    if (subjects.length !== 4) {
      return NextResponse.json({ error: "JAMB ExamMode requires exactly 4 subjects" }, { status: 400 });
    }
    questionsToGenerate = [
      { subject: subjects[0], numQuestions: 60 }, // English
      { subject: subjects[1], numQuestions: 40 },
      { subject: subjects[2], numQuestions: 40 },
      { subject: subjects[3], numQuestions: 40 },
    ];
  } else {
    return NextResponse.json({ error: "Invalid mode or examType combination" }, { status: 400 });
  }
  ...
  try {
    ...
    for (const { subject, numQuestions } of questionsToGenerate) {
      const batchSize = 10;
      const batches = Math.ceil(numQuestions / batchSize);

      for (let batch = 0; batch < batches; batch++) {
        const questionsInBatch = batch === batches - 1 ? (numQuestions % batchSize || batchSize) : batchSize;

        // Enhanced prompt for Nigerian exam standards
        const isExamMode = mode === "ExamMode";
...
Generate exactly ${questionsInBatch} multiple choice questions for ${subject} that match the actual ${examType} standard and difficulty level${isExamMode ? ' for examination mode' : ''}.

CRITICAL REQUIREMENTS:
1. Questions MUST reflect actual ${examType} ${subject} curriculum topics
2. Difficulty level MUST match typical ${examType} standards${isExamMode ? ' and be suitable for formal examinations' : ''}
...
Where correctAnswer is the index (0-3) of the correct option.
Generate exactly ${questionsInBatch} questions now.`;
        ...
        const resp = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              { 
                role: "user", 
                content: `Generate ${questionsInBatch} high-quality ${examType} ${subject} examination questions now. Ensure they match the actual exam standard used in Nigerian schools${isExamMode ? ' for examination mode' : ''}.` 
              }
            ],
        ...
        const validated = questions.map((q: any) => {
          ...
          if (!Array.isArray(q.options) || q.options.length !== 4) {
            console.warn(`Question for ${subject} has invalid options`);
            return null;
          }
    ...
    console.log(`Successfully generated ${allQuestions.length} questions for ${examType} ${mode}`);
